...
export class NotificationComponent implements OnInit {
  ...
  addComponent(componentClass: Type<any>, config) {
    ...
    component.instance.divStyle = {
      'background-color': config.status === 'error' ? '#f2dede' : config.status === 'success' ? 'rgb(0, 234, 255)' : config.status === 'info' ? '#d9edf7' : '#fcf8e3'
    };
    component.instance.style = {
      ...
      fontSize:'18px',
      fontFamily:'Roboto',
      'background-color': config.status === 'error' ? '#fe4949' : config.status === 'success' ? 'rgb(0, 234, 255)' : config.status === 'info' ? '#1d9fb4' : '#daae00',
